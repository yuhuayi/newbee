...
public class ScrollTextView extends TextView {
	 ...
	 * 
	 * switch (event.getAction()) { case MotionEvent.ACTION_DOWN: distanceY =
	 * lastY = event.getY(); distanceX = event.getX(); pause();
	 * 
	 * return true; case MotionEvent.ACTION_MOVE: float dy = event.getY() -
	 * lastY; lastY = event.getY(); // currentY = currentY + dy; Message msg =
	 * Message.obtain(); msg.what = 1; msg.arg1 = (int)dy;
	 * handler.sendMessage(msg); return true; case MotionEvent.ACTION_UP: case
	 * MotionEvent.ACTION_CANCEL: goOn(); float y = event.getY() - distanceY;
	 * float x = event.getX() - distanceX;
	 * 
	 * if (Math.sqrt(y * y + x * x) < performUpScrollStateDistance) {
	 * updateScrollStatus(); } return true;
	 * 
	...
	public void generateTextList(int MaxWidth) {
		...
		/*
		 * while (!remain.equals("")) { String line = getLineText(MaxWidth,
		 * remain); lineStrings.add(line); remain =
		 * remain.substring(line.length(), remain.length());
		 * 
		 * }
		 */
	}
